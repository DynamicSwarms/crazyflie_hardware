...
class Gateway(Node):
    ...
    def add_crazyflie(self, req, resp):
        ...
        cf_config = os.path.join(get_package_share_directory('crazyflie_hardware'),
                          'launch',
    ...
    async def create_cf(self, id, channel, args):
        path = get_executable_path(package_name="crazyflie_hardware", executable_name="crazyflie")
        cmd = [path] + args
