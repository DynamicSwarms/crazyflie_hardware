#include "crtp_logic/generic_commander_logic.hpp"
#include <cmath>
...
void GenericCommanderLogic::send_full_state_setpoint(
    ...
    auto packet = packer.send_full_state_setpoint(x, y, z, vx, vy, vz, ax, ay, az, orient_comp, rr, pr, yr);
    link->send_packet_no_response(&packet);
}

void GenericCommanderLogic::send_position_setpoint(float x, float y, float z, float yaw) {
    auto packet = packer.send_position_setpoint(x, y, z, yaw);
    link->send_packet_no_response(&packet);
}