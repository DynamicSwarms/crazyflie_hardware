...
class LinkLayerPacker(Packer):
    ...
    def __init__(self, CrtpPacker):
        super().__init__(CrtpPacker, self.PORT_LINK)

    def echopacket(self):
        data = struct.pack('<')
        return self._prepare_packet(channel=self.CHANNEL_ECHO,
                                    data=data)
    
    def sourcepackt(self):
        data = struct.pack('<')
        return self._prepare_packet(channel=self.CHANNEL_SOURCE,
                                    data=data) 

    def sinkpacket(self):
        data = struct.pack('<')
        return self._prepare_packet(channel=self.CHANNEL_SINK,
                                    data=data)

    def nullpacket(self):
        data = struct.pack('<')
        return self._prepare_packet(channel=self.CHANNEL_NULL,
                                    data=data)   

    def get_vbat(self):
        data = struct.pack('<BB',
                          self.BOOTLOADER_HEADER,
                          self.BOOTLOADER_CMD_GETVBAT)
        return self._prepare_packet(channel=self.CHANNEL_LINK,
                                    data=data)
