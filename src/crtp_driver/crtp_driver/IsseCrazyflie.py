...
sys.path.append(os.path.join(os.path.dirname(sys.path[0]), 'crazyflie-lib-python'))
#cflib = importlib.import_module("..crazyflie-lib-python/cflib/crazyflie")
...
class IsseCrazyflie():
    ...
    def __init__ (self, link =None):
        ...
        self.state = IsseCFState.DISCONNECTED
    ...
    def _link_error_cb(self, errmsg):
        ...
        if (self.link is not None):
            self.link.close()
        self.link = None
        if (self.state == IsseCFState.INITIALIZED):
            self.connection_failed.call(self.link_uri, errmsg)
        if (self.state == IsseCFState.CONNECTED or
                self.state == IsseCFState.SETUP_FINISHED):
    ...
    def _check_for_initial_packet_cb(self, data):
        ...
        answering.
        """
        self.state = IsseCFState.CONNECTED
    ...
    def open_link(self, link_uri):
        ...
        self.state = IsseCFState.INITIALIZED
