...
__all__ = ['IsseRadioDriver']

logger = logging.getLogger(__name__)

_nr_of_retries = 100      ### More performance to be gained
_nr_of_arc_retries = 3   ### CHange this for more performance
...
class IsseRadioDriver(CRTPDriver):
    ...
    def parse_uri(uri: str):
        ...
        if not uri.startswith('isseradio://'):
...
class _RadioDriverThread(threading.Thread):
    ...
    def run(self):
        ...
        while (True):
            ...
            outPacket = None
            while outPacket is None and not self._sp:
                try:
            ...
            if outPacket:
                if not hasattr(outPacket, 'header'):
                    #non crtp pakcet
                    for x in outPacket:
                        dataOut.append(x)
                    continue
